                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9 text-secondary">
                                    <input type="number" min="0" class="form-control @error('actual_price') is-invalid @enderror"
                                           ...
                                           value="{{ old('actual_price', $product->actual_price ?? $product->price) }}" required />
                                ...
                                <div class="col-sm-9 text-secondary">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="has_discount" name="has_discount" value="1"
                                            {{ old('has_discount', $product->has_discount ?? false) ? 'checked' : '' }}>
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-4">
                                            <select id="discount_type" name="discount_type"
                                                ...
                                                @php
                                                    $oldType = old('discount_type', $product->discount_type ?? 'percent');
                                                @endphp
                                                <option value="percent" {{ $oldType === 'percent' ? 'selected' : '' }}>Percent (%)</option>
                                                <option value="fixed"   {{ $oldType === 'fixed' ? 'selected' : '' }}>Fixed (MMK)</option>
                                            </select>
                            ...
                            {{-- Final price (auto) + hidden price for server --}}
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9 text-secondary">
                                    <input type="number" id="price_preview" class="form-control"
                                           value="{{ old('price', $product->price) }}" readonly />
                                    <small class="text-muted">Calculated: <code>Actual Price − Discount</code></small>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9 text-secondary">
                                    @if($product->image)
                                        <img src="{{ asset($product->image) }}" alt="Current Image" style="max-width: 200px; margin-bottom: 10px;" />
                                    @endif
    ...
    $(document).ready(function () {
        ...
        function computeFinal(){
            ...
            if ($has.is(':checked')) {
                ...
                if (t === 'percent') {
                    v = clamp(v, 0, 100);
                    final = base - Math.floor(base * v / 100);
                } else if (t === 'fixed') {
                    final = base - Math.min(base, v);
                }
            }
            if (final < 0) final = 0;
            $prev.val(final);
