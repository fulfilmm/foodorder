                    ...
                    <div class="table-responsive">
                        <table id="orders-table" class="table table-bordered align-middle">
                            ...
                            <tbody id="orders-table-body">
                                @include('manager.orders.partials.orders_table_rows', ['orders' => $orders])
                            </tbody>
                        </table>
                    </div>
...
@section('custom-js')
    ...
    <script>
      ...
      flatpickr("#filter-year", { plugins:[new monthSelectPlugin({ shorthand:true, dateFormat:"Y", theme:"light" })] });
    ...
    <script>
      ...
      function applyStatusGuards(scope=document){
        scope.querySelectorAll('#orders-table .change-status').forEach(sel=>{
          ...
          sel.disabled = isTerminal(current); // cannot change after done/canceled
        });
      }
    </script>
    ...
    <script>
      ...
      function initDataTable() {
        ...
        // Re-apply guards after DT init
        applyStatusGuards(document);
      }

      function clearOtherFilters(except) {
        const fields = ['date', 'week', 'year', 'range'];
        fields.forEach(f => { if (f !== except) $('#filter-'+f).val(''); });
      }

      function fetchAndRender(payload={}) {
        $.ajax({
          url: "{{ route('manager.orders.filter') }}",
          type: "GET",
          data: payload,
          beforeSend: function(){
          ...
          error: function(){ toastr.error('Something went wrong while fetching orders.'); }
        });
      }
      ...
      $(document).ready(function () {
        ...
        $('#filter-date').on('change', ()=>clearOtherFilters('date'));
        $('#filter-week').on('change', ()=>clearOtherFilters('week'));
        $('#filter-year').on('change', ()=>clearOtherFilters('year'));
        $('#filter-range').on('change', ()=>clearOtherFilters('range'));
    ...
    <script>
      ...
      $(document).on('change', '#orders-table .change-status', function(){
        ...
        // guard transitions
        const allowed = new Set([prev, ...allowedFrom(prev)]);
        if (!allowed.has(next)) {
          $sel.val(prev);
        ...
        if (isTerminal(next)) {
          const $siblings = isMain
            ? $(`tr[data-parent-id="${id}"]`).filter(function(){
                ...
                return !isTerminal(s); // open add-ons only
              })
            : $();
      ...
      function performStatusUpdate($select, orderId, next, prev, silent=false){
        ...
        return new Promise(resolve=>{
          $.ajax({
            url: `/manager/orders/${orderId}/status`,
            type: 'PATCH',
      ...
      // After any Ajax table reload, enforce guards again
      document.addEventListener('ajaxTableReloaded', ()=>applyStatusGuards(document));
    </script>
@endsection
